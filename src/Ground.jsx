...
const Ground = () => {
  ...
  const gridMap = useLoader(
    TextureLoader,
    process.env.PUBLIC_URL + "/pics/grid.png"
  );

  const aoMap = useLoader(
    TextureLoader,
    process.env.PUBLIC_URL + "/pics/ground-ao.png"
  );

  const alphaMap = useLoader(
    TextureLoader,
    process.env.PUBLIC_URL + "/pics/alpha-map.png"
  );
  ...
  //Wordt uitgevoerd nadat de app gerenderd is (react hook ofc), past de geometry aan van de mesh van de grond om nog meer texture data te hebben
  useEffect(() => {
  ...
  return (
    <>
      ...
      {/* We gebruiken mesh hier op een standaard manier door geometry en material als children van de mesh te assignen */}
      <mesh
        // We hebben ook een reference nr dit mesh object nodig, omda van zodra dit beschikbaar wordt moeten we properties van de geometry toevoegen
        ref={meshRef}
      ...
      >
        ...
        {/* Zorgt dat alle objecten gereflecteerd worden op gornd */}
        <MeshReflectorMaterial
          ...
          mixContrast={1.5} // Contrast of the reflections
